...
Furthermore, the crime rate is correlated with many of the variables: e.g. negatively with It is negatively correlated with e.g.medv and dis, positively with e.g. housing values and distances to employment centers, and positively rad and tax.
...
```{r, results='asis',fig.align="center", warning=FALSE, message=FALSE}
...
In comparison to the previous summary, the values have decreased and all mean values are converted to zero and standard deviations to 1.
...
Nitrogen oxides concentration (nox) is by far the longest arrow and it divides the data frames observations into two separate areas. From the other variables it is hard to say properly. Presumably rad stands out, but barely. From the output below the same can be seen.
