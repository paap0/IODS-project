...
## **4<sup>th</sup> WEEK**: Clustering and classification
...
```
We are especially interested in the variable *crime*. Crime rate varies a lot between areas ranging from min=0.01 to max=88.98. Quite a few high outlier values among the 506 observations as can be seen in the plot contribute to the low  average value of 3.61 and median value of 0.26. The distribution is strongly skewed to the left.
...
To me the scatter plots are not that informative, though. Thus, I will try another approach where the correlation chart presents simultanously both the direction (color) and the magnitude (size of the circle) as well as the values of the correlation. 
...
pairs(Boston, col = km$cluster, lower.panel = NULL)
```
...
lda.arrows <- function(x, myscale = 1, arrow_heads = 0.1, color = "red", tex = 0.75, choices = c(1,2)){
  ...
  arrows(x0 = 0, y0 = 0, 
         ...
         y1 = myscale * heads[,choices[2]], col=color, length = arrow_heads)
  text(myscale * heads[,choices], labels = row.names(heads), 
       cex = 2, col=color, pos=3)
}
plot(lda.fit2, dimen = 2, col= classes, pch=classes)
lda.arrows(lda.fit2, myscale = 2)
```

Nitrogen oxides concentration (nox) is by far the longest arrow deviding the data frame observations into two separate areas. From the other variables it is hard to say properly. Presumably rad stands out, but barely. From the output below by looking at the different linear discriminants and the proportion of trace the same can be seen.

```{r message=FALSE, warning=FALSE, fig.width=5,fig.height=6, fig.align="center"}
print(lda.fit2)
```
